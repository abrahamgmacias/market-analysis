...
if file_data['switches']['token_solver'] == True:
    ...
    # Check if offload file is empty - Overwrite if it is
    try:
        pd.read_excel(file_data['offload_file'])
    except ValueError:
        status = True
    else:
        status = False
    ...
    excel = ExcelWriter(f'{excel_offload_file}.xlsx', file_data['main_table_data']['main_table'], overwrite_sheets=status)
